...
class PhotosEZForm(Toplevel):
    ...
    def __init__(self, master = None):
        ...
        self.title('Quizzaz - Kelola Quiz - Pemula')
        ...
        #self.tree.column("#0", width=650 ,stretch=NO)
        #self.tree.column("#1", width=500 ,stretch=NO)
        self.tree.heading('#0', text = 'FOTO', anchor = CENTER)
        self.tree.heading('#1', text = 'ID', anchor = CENTER)
        #self.tree.heading('#2', text = 'JAWABAN', anchor = CENTER)

        treestyle = ttk.Style()
        treestyle.configure('Treeview', rowheight=100)

        # Buttons
        #ttk.Button(text = 'DELETE', command = self.delete_product).grid(row = 5, column = 0, sticky = W + E)
        ttk.Button(self, text = 'EDIT', command = self.edit_product).grid(row = 5, column = 1, sticky = S + E + W)
    ...
    def get_products(self):
        ...
        # getting data
        query = 'SELECT qz_photo, id FROM quiz_easy'
        db_rows = self.run_query(query)
        # filling data
        imglist = []
        for row in db_rows:
            img_byte = BytesIO(row[0])
            img_call = Image.open(img_byte)
            resized = img_call.resize((150, 75), Image.ANTIALIAS)
            img = ImageTk.PhotoImage(resized)
            self.tree.insert('', 'end', image=img, values=row[1:])
            imglist.append(img)
    ...
    def edit_records(self, new_quiz, quiz, answer):
        query = "UPDATE quiz_easy SET quiz = ? WHERE quiz = ? AND answer_tohelp = ?"
