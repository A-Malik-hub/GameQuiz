...
def get_data_010():
    ...
    c = conn.cursor()
    c.execute("SELECT qz_photo FROM quiz_easy WHERE id='qz_ez_010'")
    photo_ez_010 = c.fetchone()[0]
    img_byte = BytesIO(photo_ez_010)
    ...
    #    img = Label(qz_010_frame, image=render)
    #    img.image = render
    #    img.place(bordermode=OUTSIDE)
    conn.commit()
    conn.close()
...
def ez_photoedit():
    ...
    #Command to show
    #img_byte = BytesIO(get_data_010())
    #img_call = Image.open(img_byte)
    #resized = img_call.resize((150, 100), Image.ANTIALIAS)
    #global show_photo_ez_010
    #show_photo_ez_010 = ImageTk.PhotoImage(resized)
    foto = show_photo_ez_010.__getattribute__()
    show_image = Label(qz_010_frame, image=foto)
