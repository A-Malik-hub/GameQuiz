...
class UserForm(Toplevel):
    ...
    def __init__(self, master = None):
        ...
        self.title('Quizzaz - Kelola User')
        self.configure(bg="#1a237e")
        # Creating Add User Frame Container
        frame = LabelFrame(self, text = 'Tambah User')
        ...
        self.tree.heading('#0', text = 'Username', anchor = CENTER)
        self.tree.heading('#1', text = 'Password', anchor = CENTER)

        # Buttons
        ttk.Button(self, text = 'DELETE', command = self.delete_product).grid(row = 5, column = 0, sticky = W + E)
    ...
    def edit_product(self):
        ...
        # Old Price
        Label(self.edit_wind, text = 'Old Password:').grid(row = 2, column = 1)
        Entry(self.edit_wind, textvariable = StringVar(self.edit_wind, value = old_price), state = 'readonly').grid(row = 2, column = 2)
        # New Price
        Label(self.edit_wind, text = 'New Password:').grid(row = 3, column = 1)
